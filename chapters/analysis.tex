...
\begin{table}[htp]
	\caption{The predicted number of signal ($\mnul = \frac{1}{2}\mWR$) and background events, and the prediction uncertainties in 
		several $\Mlljj$ windows.  The uncertainties are expressed in number of events. BG = Total Background, DY = \DY }
...
The $\Meejj$ and $\Mmumujj$ distributions found in data events were compared to those in predicted background events, initially with 
(Table \ref{tab:expAndObsEvtsWithAllUncs}) and without (Figure \ref{fig:obsAndExpMlljj}) $\Mlljj$ window selection criteria.  No 
statistically significant excess of data events relative to the background was found, so limits on 
...
\begin{table}[htp]
	\caption{The predicted number of signal ($\mnul = \frac{1}{2}\mWR$) and background events, their uncertainties, and the number of 
		data events. BG = Total Background, DY = \DY }
...
\mnul values between 100 $\GeV$ and \mWR.  

.  Similar to reconstructed jets, jets from \PYTHIA were clustered from 
