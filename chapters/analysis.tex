...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Results and their Uncertainties}
...
\clearpage
\begin{table}[htp]
	\caption{The predicted number of signal ($\mnul = \frac{1}{2}\mWR$) and background events, and the prediction uncertainties in 
		several $\Mlljj$ windows.  The uncertainties are expressed in number of events. BG = Total Background}
...
The $\Meejj$ and $\Mmumujj$ distributions found in data events were compared to those in predicted background events with 
(Table \ref{tab:expAndObsEvtsWithAllUncs}) and without (Figure \ref{fig:obsAndExpMlljj}) $\Mlljj$ window selection criteria applied.  
No statistically significant excess of data events relative to the background was found, so limits on 
$\sigma(\WR) \times BR(\WR \rightarrow \ell\ell jj)$ were calculated.

\clearpage
\begin{table}[htp]
	\caption{The predicted number of signal ($\mnul = \frac{1}{2}\mWR$) and background events, their uncertainties, and the number of 
		data events. BG = Total Background}
...
The difference between the selection efficiency $\chi^'$, without trigger and offline ID criteria, and $\chi$ was expected to be small.  
