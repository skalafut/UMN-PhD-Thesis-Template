...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The size of each bin in the set of $\Mlljj$ bins was optimized based on the shape and normalization of the $\Mlljj$ 
distribution found in predicted background and simulated \WR events.  $\WR \rightarrow \ell\ell jj$ events were simulated 
for signal hypotheses that had $\mnul = \frac{1}{2}\mWR$ and \mWR stepping from 0.8 to 6 $\TeV$ in increments of 0.2 $\TeV$.  
The shape of the $\Mlljj$ distribution found in \WR events is not .

%RESUME HERE
%why was MNu = MWR/2 used? because Mlljj distribution shape is not sensitive to MNu, and MNu = MWR/2 has highest selection efficiency

%why MWR increments of 0.2 TeV, instead of smaller value? For MWR = some X >= 2 TeV, the Mlljj distributions for two signal hypotheses 
%that have MWR = X and X+0.2 TeV overlap by at least 80 percent (show a figure).  The Mlljj bin optimized for MWR = X will work 
%for other hypotheses that have MWR up to X+0.2 TeV


Smaller increments in .

The sizes of the $\Mlljj$ bins, used to interpret data in the context of.
