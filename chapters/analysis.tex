...
$e\mu$ data events that were used to estimate the top quark backgrounds:
...
of the number of events for all 3200 iterations in each $\Mlljj$ window were made, of which an example  
...
to energy and ID uncertainties.  The effect of the these uncertainties on background and signal 
estimates in one $\Mlljj$ window is shown in Table \ref{tab:impactOfEnergyIdUncerts}.
...
\begin{table}[ht]
	\caption{Impact of energy and ID uncertainties on expected signal and background event counts in the $\Mlljj$ 
		...
		estimates, the estimate of top quark backgrounds in $ee$ and $\mu\mu$ channels is sensitive to electron and muon energy and ID uncertainties.}
  ...
  \centering
    \begin{tabular}{c|c|c|c}
	 ...
	 %Top quark backgrounds & statistical & $44$ & $44$  \\
  \hline
  \end{tabular}
\end{table}


%the number of expected events in a window is obtained before any toys are thrown, and the statistical uncertainty is calculated using the formula listed below
%
%The statistical uncertainty is equal to $\sqrt{\sum w_{i}^{2}}$, where $w_{i}$ is the weight of event i.  For example, if an electron channel cut and count bin has 10 events
%from data in the \FS CR, the statistical uncertainty on the top background in this bin would be $0.43 \cdot \sqrt{10}$, since data events all have a weight of 1.
%When there are zero expected events, such as in the \FS CR for $\Mlljj > 2.7\TeV$, the Gamma function parameters are
%modified as follows:
%
%\begin{itemize}
%	\item N is set to 0
%	\item $\alpha$ is the average event weight, equal to weighted events / unweighted events, with $\Mlljj > 600\GeV$.
%	\item the statistical contribution to $\sigma$ is set to $\alpha$.  Thus when there are zero weighted events, the statistical uncertainty is $\alpha$.
%\end{itemize}
