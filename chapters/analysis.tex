...
Limits on \WR production were calculated to design a structure that facilitated tests of different 
\mWR hypotheses, and to interpret events from data and simulations 
...
\begin{equation}
	Poisson(\mu s(\pmb{\theta}) + b(\pmb{\theta}))
\end{equation}

where $\mu$ is the signal strength, and $\pmb{\theta}$ are the expected event uncertainties 
described in detail in Section \ref{sec:uncertainties}.  Using Bayesian statistics, the probability 
distribution for $\mu$ given $\Nlljj$ measured events, $p(\mu|\Nlljj)$, is calculated using\cite{bayesianDataAnalysis}:

\begin{equation}
	p(\mu|\Nlljj) = \int p(\mu|\pmb{\theta},\Nlljj)p(\pmb{\theta}|\Nlljj)d\pmb{\theta}
\end{equation}

where $p(\pmb{\theta}|\Nlljj)$ are the probability distributions for the uncertainties given 
the measurement $\Nlljj$ (``marginal posterior distributions''), and $p(\mu|\pmb{\theta},\Nlljj)$ 
are the probability distributions for the measurement $\Nlljj$, and the signal strength $\mu$ given 
the uncertainties $\pmb{\theta}$ (``conditional posterior distributions'').  Functional forms 
...
over $\pmb{\theta}$ were evaluated numerically using \MC simulations.  Following \MC integration, 
...
\begin{itemize}
	...
	\end{itemize}
	\item The window that minimized the $\sigma_{WR} \times BR(\WR \rightarrow \ell\ell jj)$ upper limit was selected as the optimal window.
\end{itemize}
...
signal and all SM backgrounds were counted, and these numbers were compared in search of a signal.  
...
cross section uncertainty resulted from uncertainties on SM coupling constants and particle masses 
hard coded in \PYTHIA.  The magnitudes of uncertainties on SM parameters were small, and as a result 
...
New pileup weights $w_{PUj}$ for simulated events were calculated after the upward and downward shifts, and 
...
backgrounds, so limits were calculated on $\sigma_{WR} \times BR(\WR \rightarrow \ell\ell jj)$, and \mWR and \mnul.
...
Upper limits on $\sigma_{WR} \times BR(\WR \rightarrow \ell\ell jj)$ at 95\% CL were calculated in each $\Mlljj$ 
...
on $\sigma_{WR} \times BR(\WR \rightarrow \ell\ell jj)$ for fixed $\mnul = \frac{1}{2}\mWR$ were extrapolated into 
\mnul and \mWR exclusion limits using a procedure described next.

\begin{figure}[btp]
	\centering
	\subfigure{
		\includegraphics[width=0.45\textwidth]{figures/limWReejj_SHv19700toysAprilTwentyThree.png}
	}
	\subfigure{
		\includegraphics[width=0.45\textwidth]{figures/limWRmumujj_SHv19700toysAprilTwentyThree.png}
	}
	\label{fig:oneDimLimits}
	\caption{Limits on $\sigma_{WR} \times BR(\WR \rightarrow \ell\ell jj)$ at 95\% CL versus \mWR hypothesis.  The $ee$ ($\mu\mu$) channel 
	is shown on the left (right).}
\end{figure}

The limits on $\sigma_{WR} \times BR(\WR \rightarrow \ell\ell jj)$ for fixed $\mnul = \frac{1}{2}\mWR$ are proportional 
to the \WR signal efficiency $\chi$ of the event selection, but are not dependent on the shape of the \WR signal distribution 
within any $\Mlljj$ window.  Thus, a limit at $(\mWR_{1}, \mnul_{1} = \frac{1}{2}\mWR_{1})$ can be extrapolated to a limit at any 
other point $(\mWR_{2}, \mnul_{2} \neq \frac{1}{2}\mWR_{2})$ by knowing the difference in signal efficiency $\chi$ between the two 
points:

%no type of multiplication sign looks good between the fraction and the Limit written on the right side of this equation
\begin{equation}
	Limit[(\mWR_{2}, \mnul_{2} \neq \frac{1}{2}\mWR_{2})] = \frac{\chi[(\mWR_{2}, \mnul_{2} \neq \frac{1}{2}\mWR_{2})]}{\chi[(\mWR_{1}, \mnul_{1} = \frac{1}{2}\mWR_{1})]} \quad Limit[(\mWR_{1}, \mnul_{1} = \frac{1}{2}\mWR_{1})]
\end{equation}

In general $\chi$ increases for increasing \mWR, as a heavier initial \WR translates into more energetic final state 
leptons and jets.  For a fixed value of \mWR, variations in $\chi$ with \mnul were determined by .
