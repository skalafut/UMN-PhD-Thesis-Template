...
\begin{equation}
	Poisson(\mu s(\textbf{\theta}) + b(\textbf{\theta}))
\end{equation}
...
distribution for $\mu$ given $\Nlljj$ measured events, $p(\mu|\Nlljj)$, is calculated as \cite{bayesianDataAnalysis}:
...
events after all selections.  The initial comparison between expected (simulated) and observed results was made 
by inspecting the $\Meejj$ and $\Mmumujj$ distributions with (Table \ref{tab:expAndObsEvtsWithAllUncs}) and without (Figure 
\ref{fig:obsAndExpMlljj}) the $\Mlljj$ window cuts.  No 
