...
0.982 and an $\Et,\eta$ dependent weight, and was found to be 2\%.  The uncertainties on lepton reconstruction and trigger 
selection efficiencies were included in limit calculations using log-normal distributions for the marginal posterior distributions.
...
energy scales.  Uncertainties on these weights lead to uncertainties on the number of selected \DY and \WR events, whose 
...
The efficiency in data to reconstruct an interaction as a vertex could not be measured exactly, so an analytic, multivariate 
function was used to approximate this efficiency in data.  This function was used to derive the pileup weights\footnote{The pileup 
...
to simulated events, so the uncertainties of this function's parameters lead to uncertainty in $w_{PU}$.  The $w_{PU}$ uncertainty 
...
calculated, and the $w_{PU}$ uncertainty was calculated as the maximum difference in total event weight between the original and 
the new weights.  For both \DY and \WR events, the pileup uncertainty was 3\% or less in all $\Mlljj$ windows.
