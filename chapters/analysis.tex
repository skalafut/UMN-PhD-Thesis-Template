...
statistics in $e\mu$ data, and discrepancies between data and simulations in control 
...
limit on the signal strength, and this value was scaled into a limit on $\sigma(\WR) \times BR(\WR \rightarrow \ell\ell jj)$.
...
the tails grew larger as \mWR increased, as shown in Figure \ref{fig:signalShapesAfterSelection}.  
...
$\ell$ ID & Uncertainty on $\ell$ ID weights applied to simulated events, see Chapters \ref{sec:muonRecoAndSelection,sec:electronRecoAndSelection}. \\
  \hline
  \end{tabular}
\end{table}
...
New pileup weights $w_{PU,j,\Uparrow}$ and $w_{PU,j,\Downarrow}$ for simulated events were calculated after the upward and downward shifts, and 
...
\begin{figure}[tp]
  ...
  \subfigure{
    \includegraphics[width=0.55\textwidth]{figures/lim2dWReejj_SHv19700toysAprilTwentyThree_exclusionOverlayWithExpPlusMinusOneSigma.png}
  }
  \subfigure{
    \includegraphics[width=0.55\textwidth]{figures/lim2dWRmumujj_SHv19700toysAprilTwentyThree_exclusionOverlayWithExpPlusMinusOneSigma.png}
  }
...
signal selection efficiency, $\chi \lesssim 10\%$, for any \mWR with $\mnul \lesssim \frac{1}{8}\mWR$, so in this low \mnul 
...
($ee$) channel.  These exclusion limits are consistent with expectations of the SM, and extend the mass limits in the $\mu\mu$ 
