...
\label{sec:statUnc}
The statistical uncertainty in each $\Mlljj$ window was calculated for each signal and background source as 
$\sqrt{\sum w_{i}^{2}}$, where $w_{i}$ was the weight of event i, and the sum ran over all selected events.  
The top quark background was estimated using $e\mu$ data events all with a weight of 1, while simulated \WR 
and \DY events had positive weights less than 1.
