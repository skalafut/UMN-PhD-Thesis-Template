...
\begin{table}[ht]
  ...
  \centering
    \begin{tabular}{c|c|c}
		\mWR (GeV)                     & eejj uncertainty (\%) & $\mu\mu$jj uncertainty (\%)  \\
      \hline
	  1000  & $4$ & $4$ \\
	  2200 & $7$ & $7$ \\
	  3000 & $15$ & $15$ \\
...
\begin{table}[htp]
	\caption{For \mWR hypotheses with $M_{\nul} = \frac{1}{2} \mWR$, these are the number of expected \WR signal and background events in each $\Mlljj$ window, and 
		their statistical and systematic uncertainties.  Each uncertainty is listed as a number of events relative to the expected number of events.}
	\label{tab:expectedEventsAndAllUncs}
\centering
\resizebox{1\textwidth}{9cm}{\input{uncertainty_table_withoutData}}
\end{table}
