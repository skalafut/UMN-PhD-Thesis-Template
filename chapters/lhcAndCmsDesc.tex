...
resolutions with which the lepton and jet energies and trajectories are measured.  In this chapter, the CERN LHC and 
the pp collisions it delivers are described, and is followed by descriptions of the CMS sub-detectors \cite{\cite{cmsDetectorPaper}} 
that are used to measure lepton and jet energies and trajectories.

The CERN LHC was constructed in a 27 km circular tunnel \cite{lhcTDR} to deliver pp collisions at 13 $\TeV$.  In the 
...
Particles produced at the CMS IP were detected using several sub-detectors.  Located closest to the IP was the silicon 
tracker, which was used to detect charged particles.  Surrounding the tracker were the electromagnetic (ECAL) and hadronic 
(HCAL) calorimeters, which were used to identify neutral and charged particles.  The ECAL was used to identify photons and 
tracks produced by electrons ($e^{\pm}$), and the HCAL was used to identify neutral hadrons and tracks produced by charged 
hadrons.  The tracker and both calorimeters were inside the solenoid magnet that generated a 3.8 $\unit{T}$ magnetic field.  
Muon detectors, located in the iron magnet return yoke where the magnetic field strength varied between 1 and 3 $\unit{T}$, 
were used to identify tracks produced by muons ($\mu^{\pm}$).  Each sub-detector covered 360$^{\circ}$ in the plane 
perpendicular to the proton beam axis, and was .

%RESUME HERE

Particles produced in pp collisions were 
...
charged particles were also characterized by points of origination, vertices, measured from the IP.  The tracker and muon 
detectors measured the scalar transverse momentum ($\pt$) of leptons and jets, and the calorimeters measured the scalar transverse 
...
trajectories as $\Delta R \equiv \sqrt{\eta^{2} + \phi^{2}}$.  Every charged particle detected by the tracker 
was also distinguished by an interaction vertex.  Each vertex was measured from the IP, and was defined by a transverse 
...
The pixel detector was built from $\approx$1 m$^{2}$ of small silicon detectors, each covering 100 $\times$ 150 $\mu$m$^{2}$.  
...
installed in a turbine pattern (Figure \ref{fig:pixelTracker}) \cite{pixelCommissioning}.  These pixel 
...
the barrel region silicon detectors.  The strip detector provided 5 to 14 measurements for every track, and was primarily used to 
