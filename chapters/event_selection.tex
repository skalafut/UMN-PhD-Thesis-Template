...
where two high-$p_{T}$, same flavor charged leptons (only $e^{\pm}$ and $\mu^{\pm}$), and two high-$p_{T}$ jets were reconstructed.  
...
detectors.  In events where two same flavor charged leptons, and two jets were reconstructed, 
additional selections were applied to increased the sensitivity of this search to high-mass 
\WR boson signatures.



\section{Online Event Selection}
Charged leptons that came from an interaction vertex were identified by the trigger system, which 
selected events online (during collisions) that were saved to permanent storage.  Events in 
the electron channel ($\WR \rightarrow eejj$ final state) were selected using a trigger that applied the following 
selections:

\begin{itemize}
	\item Two non-overlapping 5 $\times$ 5 ECAL crystal clusters detected $E_{T} >$ 33 GeV.
	\item For each energetic ECAL crystal cluster:
	\begin{itemize}
		\item The majority of the energy must be measured in an $\eta, \phi$ region that is two crystals wide in $\eta$.
		\item The ratio of hadronic energy in the tower behind the ECAL cluster to the ECAL cluster energy must be low, $\frac{E_{HCAL}}{E_{ECAL}} < 0.2$.
		\item The ECAL cluster energy divided by the total ECAL energy in a $\DeltaR =$ 0.3 radius cone centered on the ECAL cluster is high, $\frac{E_{cluster}}{E_{cone}} > 0.85$
		\item The total HCAL energy in a $\DeltaR =$ 0.3 radius cone centered on the ECAL cluster divided by the ECAL cluster energy is low, $\frac{E_{HCAL,cone}}{E_{ECAL}} < 0.3$
		\item The .
