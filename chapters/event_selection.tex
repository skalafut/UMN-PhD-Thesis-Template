...
own distinguishing features, shown in Table \ref{tab:pfRecoBins}.  Jets are born from
unstable quarks and gluons which decay to stable charged and neutral hadrons ($\pi^{\pm}$,
$K^{0}$), stable charged leptons ($e^{\pm}$, $\mu^{\pm}$) coming from weak boson
decays, and photons coming from electromagnetic decays of neutral hadrons like the
$\pi^{0}$.  Thus, all particles reconstructed by the PF algorithm are considered during
jet reconstruction.
