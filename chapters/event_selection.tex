...
\begin{itemize}
	...
	\item The dilepton mass $\Mll$ of the two selected leptons was $> 200$ $\GeV$.
	\item The invariant mass $\Mlljj$ of the selected leptons and jets was $> 600$ $\GeV$.
\end{itemize}
...
in one kinematic variable distribution.  A \WR and \nul produces an excess in the distributions of many kinematic variables, including 
the lepton and jet energy and invariant mass variables used to select events.  However, the position and shape of the resulting 
excess in all kinematic variable distributions is sensitive to the unknown \mnul, as well as \mWR.  By construction the shape of 
the $\Mlljj$ distribution excess has negligible sensitivity to \mnul, and the excess is centered at \mWR.  For these reasons evidence 
of a \WR and \nul was searched for as an excess above predicted backgrounds in the $\Mlljj$ distribution.

Events from ST backgrounds that passed all selections produced a $\Mlljj$ distribution that decreased rapidly with increasing 
$\Mlljj$, as shown in Figure \ref{fig:mlljjVariableOfMerit}.  Before searching for evidence of a \WR and \nul in the data, the 
$\Mlljj$ distribution from all backgrounds was predicted using data and simulated events in control regions.  In the control 
regions data and simulated events were expected to agree, and, based on the Run I search results and the assumption of lepton 
flavor conservation in $\WR \rightarrow \ell\nul$, no sign of a \WR and \nul was expected.
