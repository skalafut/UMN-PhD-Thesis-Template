...
Charged leptons that came from a proton-proton interaction were identified by the trigger system, which 
selected events online (during collisions) that were saved to permanent storage.  Events used in 
the ee channel \WR search ($\WR \rightarrow eejj$) were first selected using Level-1 triggers 
that required: 

\begin{itemize}
	\item At least 40 GeV of energy be measured in a 5 $\times$ 5 crystal region of ECAL (one supercluster, SC).
	\item Or at least 22 GeV of energy be measured in one SC, and at least 10 GeV of energy be measured in 
		another, non-overlapping SC.
\end{itemize}

Subsequently, local reconstruction was run in regions where the Level-1 triggers fired, and events 
were selected and saved to permanent storage if the following High Level trigger requirements were 
met: 

\begin{itemize}
	\item Two non-overlapping ECAL superclusters (SC) are detected with energy > 33 GeV.
	\item For each SC:
	\begin{itemize}
		\item The ratio of hadronic energy in the tower behind the SC to the SC energy must be low, $\frac{E_{HCAL}}{E_{SC}} < 0.15$.
		\item Ninety percent of the SC energy must be measured in an $(\eta, \phi)$ region that is two crystals wide in $\eta$.
		\item A reconstructed track with hits in at least two pixel tracker layers extrapolates to the $z_{SC}$ SC center within 
			2.3 \cm, and the $(\eta_{SC}, \phi_{SC})$ SC center within the $(\eta, \phi)$ area of one ECAL crystal.  This only applies to SCs in the barrel.
	\end{itemize}
\end{itemize}

A second set of ee channel events were used only to estimate backgrounds.  These events were first 
selected online using a Level-1 trigger, which required at least 30 GeV of energy be measured in a SC 
with $|\eta| < 2.1$.  Then, local reconstruction was run in regions where the Level-1 trigger fired, 
and events were selected and saved to permanent storage if the following High Level trigger requirements 
were met:

\begin{itemize}
	\item One SC is detected with energy > 30 GeV.
	\item For the SC with energy > 30 GeV:
	\begin{itemize}
		\item Ninety percent of the SC energy must be measured in an $(\eta, \phi)$ region that is two crystals wide in $\eta$.
		\item The ratio of hadronic energy in the tower behind the SC to the SC energy must be low, $\frac{E_{HCAL}}{E_{SC}} < 0.055$ in the barrel, $< 0.07$ in the endcap.
		\item In a cone of radius $\DeltaR =$ 0.3 centered on the SC ($\sim$900 crystals in the cone):
		\begin{itemize}
		...
		\end{itemize}
		\item A reconstructed track with hits in at least two pixel tracker layers extrapolates to the $z_{SC}$ SC center within 
			1 \cm, and the $(\eta_{SC}, \phi_{SC})$ SC center within the $(\eta, \phi)$ area of half of one ECAL crystal.  This applies to SCs in the barrel and endcap.
		\item the reconstructed track momentum and SC energy cannot differ by more than 50\%
	\end{itemize}
	\item A second SC is detected with energy > 4 GeV.
\end{itemize}


Events used in the muon channel \WR search ($\WR \rightarrow \mu\mu jj$) were first selected online using a Level-1 trigger 
that required at least 16 GeV of energy be measured in one muon DT or CSC detector.  Subsequently, local 
reconstruction was run in regions where the Level-1 trigger fired, and events were selected and saved to 
permanent storage if the following High Level trigger requirements were met: 

\begin{itemize}
	\item A global curve representing a muon is fit to a reconstructed track and at least one muon detector hit with $\chi^{2}/nDOF <$ 20.
	\item In the $(x,y)$ plane, the distance between the origin of the muon track and the primary vertex is below 1 \mm.
	\item The reconstructed muon track has $p_{T} >$ 50 GeV.
\end{itemize}
...
($e\mu jj$ final state) were used to estimate backgrounds in the ee and $\mu\mu$ channels.  The $e\mu$ channel events were first selected 
online using Level-1 triggers that required at least 16 GeV of energy be measured in one muon DT or CSC 
detector.  Then, local reconstruction was run in regions where the Level-1 trigger fired, and events were 
selected and saved to permanent storage if the following High Level trigger requirements were met:

\begin{itemize}
	\item A global curve representing a muon is fit to a reconstructed track and at least one muon detector hit with $\chi^{2}/nDOF <$ 20.
	\item In the $(x,y)$ plane, the distance between the origin of the reconstructed muon track and the primary vertex is below 1 \mm.
	\item The reconstructed muon track has $p_{T} >$ 30 GeV.
	\item One ECAL supercluster (SC) is detected with energy > 30 GeV.
	\item For the SC:
	\begin{itemize}
		\item The ratio of hadronic energy in the tower behind the SC to the SC energy must be low, $\frac{E_{HCAL}}{E_{SC}} < 0.15$.
		\item Ninety percent of the SC energy must be measured in an $(\eta, \phi)$ region that is two crystals wide in $\eta$.
		\item A reconstructed track with hits in at least two pixel tracker layers extrapolates to the $z_{SC}$ SC center within 
			2.3 \cm, and the $(\eta_{SC}, \phi_{SC})$ SC center within the $(\eta, \phi)$ area of one ECAL crystal.
	\end{itemize}
\end{itemize}
...
Data collected by the CMS experiment from September until November 2015, when the center-of-mass energy 
...
in combining 50 \ns and 25 \ns data, the 50 \ns data was not used in this analysis.  During the August break 
...
era Run2015D.  As in Run2015C, in Run2015D all data was collected with similar beam conditions.
...
The raw data collected by CMS is too large ($\gtrsim 10^{4}$ terabytes) to process for analyses
