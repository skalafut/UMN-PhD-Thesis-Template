...
here selected events in two sequential steps.  In the first step, one or more Level-1 triggers were 
required to fire.  Then, local reconstruction was run in the regions where Level-1 triggers fired to 
build calorimeter and muon detector energy clusters with finer granularity.  In the second step, 
a High Level trigger applied selection cuts to the calorimeter or muon detector energy clusters.  If 
an event passed these selections, tracks and interaction vertices were reconstructed and linked with 
calorimeter and muon detector energy clusters to build electron (e) and muon ($\mu$) candidates.  Final 
selections were then applied to e,$\mu$ candidates.  Specific selections used in different Level-1 
and High Level triggers are discussed next.

Events used in the ee channel \WR search ($\WR \rightarrow eejj$) were first selected by Level-1 triggers 
that required: 
...
Then, events were saved to permanent storage if the following High Level trigger requirements were met: 
...
following High Level trigger requirements were met:
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item In a cone of radius $\DeltaR =$ 0.3 centered on the SC ($\sim$900 ECAL crystals, $\sim$35 HCAL towers in the cone):
		\begin{itemize}
...
Events used in the muon channel \WR search ($\WR \rightarrow \mu\mu jj$) were first selected online using 
a Level-1 trigger that required $>$ 16 GeV of momentum be measured in a muon DT or CSC detector.  Following 
the Level-1 selection, events were saved to permanent storage if the following High Level trigger requirements 
were met: 
...
\begin{itemize}
	...
	\item In a cone of radius $\DeltaR =$ 0.3 centered on the muon detector energy cluster ($\sim$900 ECAL crystals, $\sim$35 HCAL towers in the cone):
	\begin{itemize}
...
storage if the following High Level trigger requirements were met:
