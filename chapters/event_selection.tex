...
the electron channel ($\WR \rightarrow eejj$) were selected using Level-1 triggers that required at least 
one ECAL supercluster (5 $\times$ 5 crystals) with $E_{T} >$ 40 GeV, or a pair of separated ECAL 
superclusters with $E_{T} >$ 22 and 10 GeV.  Subsequently, events were selected with the following 
requirements in a High Level trigger and saved to permanent storage: 

\begin{itemize}
	\item Two non-overlapping ECAL superclusters (SC) are detected with $E_{T} >$ 33 GeV.
	\item For each SC:
	\begin{itemize}
