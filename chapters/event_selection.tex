...
in the following multi-step procedure.  In the first step, a \MC generator, like \PYTHIA or \MADGRAPH, simulated 
...
\begin{table}[bt]
\caption{Fully reconstructed \MC samples produced by the CMS \MC production group.  The \DY and t$\bar{t}$+jets events 
...
applied to every simulated event before simulated events were analyzed further.

Independent of the CMS \MC production group, additional $\WR \rightarrow lljj$ simulated datasets using 
\PYTHIA were produced.  Only the first simulation step was run, and the \PYTHIA configuration was 
...
As shown in Table \ref{tab:lowerMuonPt}, a lower $\pt$ cut would reduce \WR sensitivity.  After the $\pt$ cut, muons 
...
%and the uncertainty on the weight was below 0.001.
...
to reject electrons that were not produced by \WR decays:
...
constituent and its $(\eta,\phi)$ distance from the jet axis defined by a charged particle track.  Most jet 
constituents fell within $\Delta R < 0.4$ of the jet axis, but it was possible for low energy constituents to 
be futher away.
...
that passed the high $\pt$ lepton and $\Mll$ selections, one or both leptons could have come from jets with $\pt \gg 100$ 
\GeV that produced an energetic lepton in the tracker.  To reduce the fraction of such events passing the kinematic 
...
of the four final state particles (\Mlljj) averaged over many events was expected to be in good agreement with \mWR predicted by 
