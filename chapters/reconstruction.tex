...
by ST processes like $\DY$+jets, and their final state leptons and jets imitate the $\WR \rightarrow \ell\ell jj$ decay signature.

\subsection{Signal and Background Characteristics}
The heavy \WR decays through a lighter \nul to a $\ell\ell jj$ final state according to the following decay chain:

The previous \WR and \nul search \cite{cmsWRRunOneResults} excluded \WR production at 95\% CL for $\mWR < 2$ $\TeV$ and $\mnul < \mWR$, 
so the \WR and \nul lifetimes, inversely proportional to their masses, are very short.  As a result, both leptons and quarks are 
promptly produced near the IP.  Subsequently the quarks hadronize into jets of stable particles, whose masses are negligible compared to 
\mWR and \mnul.
...
produced by diboson (WW, WZ, ZZ), W+jets, and QCD multi-jet processes.  The .
