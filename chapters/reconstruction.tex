...
The CMS jet energy resolution and the large expected \mWR motivated the jet $\pt$ and $\eta$ selection criteria.  The \WR is expected 
to be heavy ($\mWR > 3$ $\TeV$), so it is produced with with low momentum.  Therefore, the \WR decay produces jets in the central $|\eta|$ 
region (Figure \ref{fig:wrLeptJetEtas}).  Thus, each event must have two jets reconstructed with $|\eta| < 2.4$.  Also due to 
the large expected \mWR, the \WR decay produces jets with large $\pt$ (Figure \ref{fig:wrLeptJetPts}).  However, the quark progenitors 
of both jets have $\pt$s that decrease with lower $\mnul/\mWR$ (Figure \ref{fig:wrLeptQrkPtsVarMNu}), so the jet $\pt$ selection threshold 
should be as low as possible to maintain good sensitivity to low $\mnul/\mWR$ signals.  The $\pt$ of jets reconstructed with 
$|\eta| < 2.4$ were measured with a resolution better than 13$\pm$1\% for $\pt > 40$ $\GeV$; for $30 < \pt < 40$ $\GeV$ the resolution 
and its uncertainty worsened to $\sim$15$\pm$4\% \cite{jetResolutionInCollisions}.  To avoid low $\pt$ jets measured with large $\pt$ 
uncertainty, each event must have two $|\eta| < 2.4$ jets reconstructed with $\pt > 40$ $\GeV$.  The 40 $\GeV$ threshold was validated 
by comparing a \WR signal to ST backgrounds.  As listed in Table \ref{tab:lowerJetPtCuts}, reducing the jet $\pt$ threshold would reduce 
sensitivity to a \WR signal relative to ST backgrounds.

%the online lepton selection criteria and \WR kinematics motivate the lepton pt and eta selection criteria


%jet reco algorithm motivates lepton jet deltaR selection criteria
