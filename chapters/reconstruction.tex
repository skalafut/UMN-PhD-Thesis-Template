...
$\chi^{2} \leq$ 30 \cite{trackerPerformanceInCollisions}.  Each time a new signal is added to a track, an analytic function 
...
by lower energies or larger distances from the IP.  Using this algorithm, the .

%RESUME HERE

lose negligible energy when traversing the entire tracker.  Upon reaching the outermost 
...
lengths of material.  As a result, $\sim$35\% of electrons that traverse the tracker lose more than 70\% of their initial energies 
through bremsstrahlung \cite{trackerPerformanceInCollisions}, which cannot be measured by the tracker.  Using the iterative 
track reconstruction procedure described previously, the electron track reconstruction algorithm builds tracks starting 
with signals from the inner tracker layers, and works outward to include signals with similar or lower energies.  The 
algorithm models the electron energy loss in successive silicon layers as a sum of multiple Gaussians, and requires that 
signals in successive layers have $\chi^{2} \leq$ 2000 before a link is made.  By using this electron track reconstruction 
algorithm, electrons with $\pt > 20$ $\GeV$ and $|\eta| < 2.5$ are reconstructed in the tracker with an efficiency of 97\% 
or better \cite{gsfPerformanceInCollisions}.
