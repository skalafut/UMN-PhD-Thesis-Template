...
The \DY+jets process produces $\ell\ell jj$ final state events that have low $\Mll$, and low $\pt$ 
jets.  Before producing a dilepton system via \DY, the interacting quarks radiate two partons.  These partons are radiated with a 
probability that is independent of $\phi$ and the angle $\theta$ relative to the beam axis, but decreases rapidly with $\pt$.  These 
partons recoil against the $Z/\gamma^{*}$ to conserve the initial quark system momentum, and subsequently hadronize into jets.  Due to 
the $Z$ boson resonance, the majority of \DY events produce lepton pairs with $60 < \Mll < 120$ $\GeV$.  The upper bound $\Mll < 120$ 
$\GeV$ constrains both leptons to have $\pt \lesssim 60$ $\GeV$.  Considering many $\DY$+jet events, the lepton jet 
invariant mass ($\Mlljj$) distribution peaks below 200 $\GeV$ due to the $Z$ boson resonance.  However, events where the $Z$ boson 
has significant momentum, or the $\gamma^{*}$ has significant energy create a long $\Mlljj$ tail above the peak.
...
final state is realized when two $\ell$ are produced from bottom quark and $W$ boson decays.  Although several top quark processes 
contribute to the background, the dominant contribution is from $t\bar{t}$.  In $t\bar{t} \rightarrow \ell\ell jj$ events both $W$ 
bosons decay to a charged lepton, and, due to combinatorics, the $e\mu$ final state is produced twice as often as the $ee$ or $\mu\mu$ 
final state.  The charged leptons are constrained by the $W$ mass to have $\pt \lesssim 50$ $\GeV$, and their invariant mass $\Mll$ 
can be as high as the $t\bar{t}$ rest mass.  Considering many $t\bar{t} \rightarrow \ell\ell jj$ events, the $\Mlljj$ distribution 
peaks below the $t\bar{t}$ mass because of the neutrinos from the $W$ decay.  However, events where the $t\bar{t}$ system has significant 
momentum create a long $\Mlljj$ tail above the peak.
