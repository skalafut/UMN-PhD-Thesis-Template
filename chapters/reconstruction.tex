...
Several independent reasons motivate the offline lepton and jet kinematic selection criteria.  These reasons and the resulting 
selection criteria, identical for both $eejj$ and $\mu\mu jj$ final states, are described here.

The large expected \mWR and the CMS jet $\pt$ resolution vs $\pt$ motivated the jet $\pt$ and $\eta$ selection criteria.  The \WR is 
...
to avoid concealing low $\mnul/\mWR$ signals.  The $\pt$ of jets reconstructed with $|\eta| < 2.4$ were measured with a resolution 
better than 13$\pm$1\% for $\pt > 40$ $\GeV$; for $30 < \pt < 40$ $\GeV$ the resolution worsened to $\sim$15$\pm$4\% 
\cite{jetResolutionInCollisions}.  To avoid low $\pt$ jets measured with large $\pt$ uncertainty, each event was required to have two 
$|\eta| < 2.4$ jets reconstructed with $\pt > 40$ $\GeV$.  If more than 2 jets passed these criteria, the two highest $\pt$ jets were 
used.  The 40 $\GeV$ threshold was validated by comparing a \WR signal to ST backgrounds.  As listed in Table \ref{tab:lowerJetPtCuts}, 
...
Jets are reconstructed using the anti-$k_{T}$ algorithm with distance parameter $\Delta R = 0.4$.  This parameter defines the 
characteristic length in $(\eta,\phi)$ space between two well separated constituent particles of the same jet, and puts a soft limit 
on the jet $(\eta,\phi)$ area.  However, it is possible for constituent particles in a jet to be separated from the $\pt$ averaged 
jet axis by $\Delta R > 0.4$.  To avoid a jet and lepton that are reconstructed in close proximity, each reconstructed lepton 
passing the identification, $\pt$, and $\eta$ selections was required to be separated from both selected jets by $\Delta R > 0.4$.

%WR lepton kinematics motivates dilepton mass selection criteria
The \WR decay produces one lepton through $\WR \rightarrow \ell_{1}\nul$, and a second lepton via $\nul \rightarrow \ell_{2}jj$.  Since 
both leptons are produced by the decay of massive particles ($\mnul \gtrsim 90$ $\GeV$, see Chapter \ref{sec:lrsPhenomenology}), their 
trajectories are correlated only if the \nul is boosted ($\mnul/\mWR \lesssim \frac{1}{10}$).  When their trajectories are uncorrelated 
their .

%all previous lepton and jet selection criteria create an artificial peak at Mlljj = 550 GeV, so Mlljj is required to exceed 600 GeV
