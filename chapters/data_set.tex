...
data and simulated events were corrected by changing the weight of each simulated event depending on the $\pt$ and 
$\eta$ of the selected leptons.  Independent of the kinematics of the selected leptons or jets, the weight of each simulated event 
was normalized based on the dataset size and the integrated luminosity of data, and adjusted further according to the pileup 
distribution found in data.  The pileup distribution found in the data is approximated by a Poisson distribution with mean 14 \cite{lumi}, 
but the pileup distribution found in simulated events matches a Poisson distribution with mean 12.  The discrepancy between the 
pileup distributions was corrected by changing the weight of each simulated event by a pileup-dependent value.
...
required to pass the following electron-muon HLT selection criteria:
...
to the selected data events in Figure \ref{fig:dataAndSimsInEMuChannel}.  Based on the magnitudes of different simulated backgrounds, 
more than 95\% of $e\mu jj$ events come from the top quark background.
