...
second step simulates the electronic signals that these particles generate in each sub-detector.  From these signals the third 
step simulates the reconstruction of leptons and jets using the algorithms described previously.
...
Simulated inelastic pp collisions show the best agreement with experimental data \cite{pythiaForHadronization} when the \PYTHIA 
generator \cite{pythia8,Sjostrand:2006za} is used to simulate parton hadronization, so all simulations use \PYTHIA to hadronize 
partons.  The simulated events are then .
