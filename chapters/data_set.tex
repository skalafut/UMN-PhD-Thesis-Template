...
Simulated events using any \MC generator show the best agreement with experimental data when \PYTHIA is used to simulate parton 
hadronization \cite{pythiaForHadronization}, so all simulations use \PYTHIA to hadronize partons.  Furthermore, in all simulations 
the unstable particles are decayed into quasi-stable particles, like the $\Sigma^{\pm}$ and $\pi^{\pm}$, with mean lifetime 
$\tau \gtrsim 8\times10^{-11}$ seconds.  These quasi-stable particles travel a mean distance c$\tau \gtrsim 2.4$ cm before decaying, 
and some will decay after generating a signal in the first layer of the silicon pixel tracker, located 4.4 cm from the interaction 
point \cite{cmsTdrPhysPerformance}.
...
In the second step the quasi-stable particles produced earlier are propagated through the CMS detector and the 3.8 $\unit{T}$ magnetic 
field, and GEANT \cite{geant4} simulates their sporadic decays and interactions with the detector.  These inter.
