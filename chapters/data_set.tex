...
the number of background events found in the $\Mlljj$ distribution, but did not eliminate them completely.  The 
contributions of ST backgrounds to the $\Mlljj$ distribution found in data was estimated using procedures 
described here.

%RESUME HERE make a smooth transition from intro to MC section, and from MC section to DY bkgnd



\section{Monte Carlo}
\label{sec:MC}
Monte Carlo (\MC) simulations were used to estimate the ST backgrounds, and to study lepton and jet kinematics 
in $\WR \rightarrow \ell\ell jj$ events.  The ST backgrounds, primarily $Z+jets \rightarrow \ell\ell+jets$ and 
$\ttbar+jets \rightarrow \ell\ell+jets$, produced events where 2 leptons and 2 jets were reconstructed and passed 
all selections.  These backgrounds and \WR signals at different \mWR and \mnul hypotheses were estimated using 
three step \MC simulations.
...
%each step, then move on to the next step
